...
function viewallemployees() {
  getdata(sqlviewemployee).then((results) => {
...
function viewalldepartments() {
  getdata(sqlviewdepartment).then((results) => {
...
function viewallroles() {
  getdata(sqlviewrole).then((results) => {
...
function addemployee() {
  getdata(
    `select CONCAT(first_name, " " , last_name) as manager from employee; select title from role;`
  ).then((results) => {
    inquirer
      .prompt([
        {
      ...
      .then((answer) => {
        insertdata(
          `INSERT INTO employee(first_name, last_name, role_id, manager_id) Select ?, ?, (SELECT id FROM role WHERE title = ? ) as role_id, (SELECT id FROM employee WHERE CONCAT(first_name," ",last_name) = ? ) as manager_id`,
          [answer.newFirstname, answer.newLastname, answer.role, answer.manager]
        );
        viewallemployees();
      });
  });
...
function updateemployeerole() {
  let employeeid;
  getdata(
    `select CONCAT(first_name, " " , last_name) as fullname from employee; select title from role;`
  ).then((results) => {
    inquirer
      .prompt([
        {
      ...
      .then((answer) => {
        getdata(
          `update employee set role_id = (select id from role where title="${answer.newRole}") where CONCAT(first_name, " " , last_name) = "${answer.fullname}"`
        );
        viewallemployees();
      });
  });
...
function updateemployeemanager() {
  getdata(
    ...
    inquirer
      ...
      .then((answers) => {
        if (answers.fullname === answers.newManager)
          console.log(chalk.red(`\nCannot assign self as Manager\n`));
        else
          getdata(
            `update employee set manager_id = (select * from (select id from employee where CONCAT(first_name, " " , last_name)="${answers.newManager}") a) where CONCAT(first_name, " " , last_name) = "${answers.fullname}"`
          );
        viewallemployees();
      });
  });
...
function viewemployeebymanager() {
  getdata(
    `select CONCAT(a.first_name, " " , a.last_name) as fullname from employee a inner join employee b on a.id = b.manager_id group by CONCAT(a.first_name, " " , a.last_name) ;`
  ).then((results) => {
    inquirer
      .prompt([
        {
      ...
      .then((answer) => {
        getdata(
          sqlviewemployee +
            ` where a.manager_id =(select id from employee where CONCAT(first_name, " " , last_name) = "${answer.manager}")`
        ).then((results) => {
...
function viewemployeebydepartment() {
  getdata(`select name from department;`).then((results) => {
    inquirer
      .prompt([
        {
          name: "department",
      ...
      .then((answer) => {
        getdata(
          sqlviewemployee + ` where c.name = "${answer.department}"`
        ).then((results) => {
          displaydata(
            `View Employee by Department: ${answer.department}`,
            results
          );
          mainchoice();
        });
...
function viewbudgetbydepartment() {
  getdata(`select name from department;`).then((results) => {
    inquirer
      ...
      .then((answer) => {
        getdata(
          `select c.name as department, sum( b.salary) as budget from employee a left join role b on a.role_id=b.id left join department c on b.department_id=c.id where c.name = "${answer.department}" group by c.name`
        ).then((results) => {
          displaydata(
            `View Budget by Department: ${answer.department}`,
            results
          );
          mainchoice();
        });
...
function addrole() {
  getdata(`select name from department;`).then((results) => {
    inquirer
      ...
      .then((answer) => {
        insertdata(
          `INSERT INTO role(title, salary, department_id) VALUES ("${answer.newTitle}", "${answer.newSalary}", (SELECT id FROM department WHERE name = "${answer.dept}"));`,
          ""
        );
        viewallroles();
      });
...
//Delete Role
function deleterole() {
  getdata(`select title from role;`).then((results) => {
    inquirer
      .prompt([
        {
          name: "title",
